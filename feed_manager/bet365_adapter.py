# feed_manager/bet365_adapter.py

import os
import json
from datetime import datetime

BET365_DATA_PATH = "data_sources/odds/bet365-scraper/data"

def load_odds_data(file_name=None):
    """
    Charge les données des cotes Bet365 depuis un fichier JSON.
    ...
    if file_name is None:
        files = [f for f in os.listdir(BET365_DATA_PATH) if f.endswith(".json")]
        if not files:
            return []
        files.sort(reverse=True)
        file_name = files[0]

    file_path = os.path.join(BET365_DATA_PATH, file_name)
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        return data
    except Exception as e:
        print(f"Erreur chargement cotes Bet365: {e}")
        return []
